 ...
 *
 * These APIs enable layer mirroring and remote layer hosting, which power
 * effects like cross-window/multi-view tab display, live thumbnails, and
 * screen sharing.
 *
 * ## Overview
 *
 ...
 * - `CAFilter`: Configurable Core Animation filters (e.g. Gaussian blur)
 *
 * ## What CAPortalLayer is for
 *
 * `CAPortalLayer` is ideal for:
 * - Displaying previews/thumbnails of a layer without re-rendering
 * - Showing the same content in multiple places (only one is interactive)
 * - Creating mirror windows that reflect a single source layer
 *
 * ## References
 *
 ...
 *
 * @code
 * // Create a thumbnail of a view's content
 * CAPortalLayer *portal = [CAPortalLayer layer];
 * portal.sourceLayer = sourceView.layer;
 ...
 */
@interface CALayer (CoreAnimationPrivate)
